<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<% request.setCharacterEncoding("utf-8"); %>    

<jsp:useBean id="dto" class="test.Test20DTO" scope="page"></jsp:useBean>

<jsp:setProperty name="dto" property="*"/>

<%
	String class1 = dto.getClass1();
	String class2 = dto.getClass2();
	String class3 = dto.getClass3();
	String class4 = dto.getClass4();
	String class5 = dto.getClass5();
%>

1반 ==> <%=class1 %><br>
2반 ==> <%=class2 %><br>
3반 ==> <%=class3 %><br>
4반 ==> <%=class4 %><br>
5반 ==> <%=class5 %><br>


<hr>


<%
	class1 = request.getParameter("class1");
	class2 = request.getParameter("class2");
	class3 = request.getParameter("class3");
	class4 = request.getParameter("class4");
	class5 = request.getParameter("class5");
%>

1반 --> <%=class1 %> <br>
2반 --> <%=class2 %><br>
3반 --> <%=class3 %><br>
4반 --> <%=class4 %><br>
5반 --> <%=class5 %>
